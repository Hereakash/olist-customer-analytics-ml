...
# IMPORTANT: choose a cutoff that leaves >= 90 days of future data
CUTOFF_DATE = "2018-06-01"  # try 2018-06-01 first
...
def main():
    ...
    snapshot_date = pd.to_datetime(CUTOFF_DATE).date()

    sql = """
    WITH params AS (
      SELECT (%(snap)s::date + INTERVAL '1 day')::timestamp AS cutoff_ts
    ),
    -- Build RFM features as-of cutoff using delivered orders ONLY
    base_orders AS (
      SELECT
    ...
    rfm AS (
      SELECT
        ...
        -- recency relative to cutoff date
    ...
    geo AS (
      SELECT customer_unique_id, MAX(customer_state) AS customer_state
      FROM raw.olist_customers_dataset
      GROUP BY customer_unique_id
    ),
    last_hist AS (
      SELECT customer_unique_id, MAX(delivered_ts) AS last_delivered_ts
      FROM hist
      GROUP BY customer_unique_id
    ),
    future_min AS (
      SELECT
        lh.customer_unique_id,
        MIN(bo.delivered_ts) AS next_delivered_ts
      FROM last_hist lh
      JOIN base_orders bo
        ON bo.customer_unique_id = lh.customer_unique_id
       AND bo.delivered_ts > lh.last_delivered_ts
      GROUP BY lh.customer_unique_id
    ),
    labels AS (
      SELECT
        lh.customer_unique_id,
        CASE
          WHEN fm.next_delivered_ts IS NOT NULL
           AND fm.next_delivered_ts <= (lh.last_delivered_ts + INTERVAL '90 days')
          THEN 1 ELSE 0
        END AS will_reorder_90d
      FROM last_hist lh
      LEFT JOIN future_min fm
        ON fm.customer_unique_id = lh.customer_unique_id
    )
    SELECT
      ...
      o.late_rate,
      rv.avg_review_score,
      COALESCE(g.customer_state, 'Unknown') AS customer_state,
      COALESCE(l.will_reorder_90d, 0) AS will_reorder_90d
    FROM rfm r
    ...
    LEFT JOIN geo g ON g.customer_unique_id = r.customer_unique_id
    LEFT JOIN labels l ON l.customer_unique_id = r.customer_unique_id
    ;
    """

    df = pd.read_sql(sql, engine, params={"snap": snapshot_date})
    ...
    with engine.begin() as conn:
        conn.execute(
            text("DELETE FROM mart.customer_churn_ml_dataset WHERE snapshot_date = :snap"),
            {"snap": snapshot_date},
        )
    ...
    print(f"Built mart.customer_churn_ml_dataset for snapshot_date={snapshot_date}")
    print("Rows:", len(df))
    print(df["will_reorder_90d"].value_counts(dropna=False))
